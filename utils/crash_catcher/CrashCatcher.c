...
CRASH_CATCHER_TEST_WRITEABLE uint32_t* g_pCrashCatcherCoreRegisters = (uint32_t*)0xE000E000;
...
typedef struct
{
    ...
    const CrashCatcherAssertRegisters*    pAssertRegisters;
...
static void dumpMSPandPSPandAssertPSR(const CrashCatcherAssertRegisters* pRegs);
static Object initAssertStackPointers(const CrashCatcherAssertRegisters* pAssertRegisters);

static void dumpTaskRegs(const CrashCatcherAssertRegisters *regs);
...
void AssertCatcher_Entry(const CrashCatcherAssertRegisters* pAssertRegisters)
{
    ...
    Object object = initAssertStackPointers(pAssertRegisters);
    ...
    do
    {
        ...
        dumpTaskRegs(pAssertRegisters);
        dumpMSPandPSPandAssertPSR(pAssertRegisters);
...
{
    Object object;
    object.pAssertRegisters = pAssertRegisters;
    object.info.sp = pAssertRegisters->psp;
    object.pSP = uint32AddressToPointer(object.info.sp);
    object.flags = 0;
    object.info.isSoftAssert = 1;
    return object;
}
...
static void dumpTaskRegs(const CrashCatcherAssertRegisters *regs)
{
    CrashCatcher_DumpMemory(&regs->R, CRASH_CATCHER_BYTE, sizeof(struct TaskRegs));
}
...
static void dumpMSPandPSPandAssertPSR(const CrashCatcherAssertRegisters* pRegs)
{
    uint32_t msp = pRegs->msp;
    uint32_t psp = pRegs->psp;
    uint32_t assertPSR = pRegs->assertPSR;
    ...
    CrashCatcher_DumpMemory(&assertPSR, CRASH_CATCHER_BYTE, sizeof(uint32_t));
}
...
static void dumpFloatingPointRegisters(const Object* pObject)
{
    ...
    int assert = pObject->info.isSoftAssert;
    if (!assert && areFloatingPointRegistersAutoStacked(pObject))
    {
...
static void dumpFaultStatusRegisters(void)
{
    uint32_t                 faultStatusRegistersAddress = (uint32_t)(unsigned long)g_pCrashCatcherCoreRegisters;
    CrashCatcherMemoryRegion faultStatusRegion[] = { {faultStatusRegistersAddress,
                                                      faultStatusRegistersAddress + 0x1000,
                                                      CRASH_CATCHER_WORD},
