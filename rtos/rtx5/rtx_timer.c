...
FLASH_TEXT_LOC
static void show_timer_stats(os_timer_t *p)
{
    if (p->name)
...
#if (defined(OS_OBJ_MEM_USAGE) && (OS_OBJ_MEM_USAGE != 0))
    REL_TRACE_NOTS(0, "--- Timer Mem: alloc=%d free=%d used=%d", osRtxTimerMemUsage.cnt_alloc, osRtxTimerMemUsage.cnt_free, osRtxTimerMemUsage.max_used);
#endif
    rt_timers_scan();
