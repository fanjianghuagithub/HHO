...
ifneq ($(GIT_REVISION),)
ifeq ($(WIN_PLAT),y)
GIT_REVISION := $(GIT_REVISION)$(shell (git diff --quiet && git diff --cached --quiet) >nul 2>&1)
else
GIT_REVISION := $(GIT_REVISION)$(shell (git diff --quiet && git diff --cached --quiet) >/dev/null 2>&1)
endif
endif
...
ifeq ($(SDK_TEST),1)
FLASH_SIZE_M := $(subst 00000,,$(FLASH_SIZE))
FLASH_SIZE_M := $(subst 0x,,$(FLASH_SIZE_M))
FLASH_SIZE_M := $(subst 0X,,$(FLASH_SIZE_M))

ifeq ($(OTA_CODE_OFFSET),)
FLASH_OFFSET_INFO := 
else 
FLASH_OFFSET_INFO := _offset_$(OTA_CODE_OFFSET)
endif
...
OTA_CHIP_TYPE :=
ifneq ($(OTA_BOOT_SIZE),) 
OTA_CHIP_TYPE := _$(CHIP)
endif
ifneq ($(APP_IMAGE_FLASH_OFFSET),) 
OTA_CHIP_TYPE := _$(CHIP)
endif

IMAGE_FILE ?= $(notdir $(T)$(OTA_CHIP_TYPE)_$(GIT_REVISION)_$(FLASH_SIZE_M)M$(FLASH_OFFSET_INFO)$(SDK_TEST_INFO)).elf
$(info REVISION_INFO: $(IMAGE_FILE))
else 
