...
IQMismatchPreprocessState *IQMismatchPreprocessState_init(int fft_size)
{
    ...
    if (st){
        ...
        BT_DRV_TRACE(5,"st:%p, M0data:%p, Table0:%p, Table1:%p, Table2:%p", st, st->M0data, st->Table0, st->Table1, st->Table2);
...
#else
    WRITE_REG(0x7fffffff,0xd0350208,0x0);//iq_cali
    val = READ_REG(0xd0350208,0x0);
#endif
...
#else
    WRITE_REG(0x7fffffff,0xd0350228,0x0);//iq_cali
    val = READ_REG(0xd0350228,0x0);
#endif
...
#ifdef IQ_CALI_TEST
    val &= 0xfeffffff; //bit25
#endif
    val |= (0x1<<24);
#if 1//def IQ_CALI_TEST
    val &= 0xffcfffff;
    val |= (0x3<<20);
#endif
...
void bt_IQ_DC_Mismatch_Correction_Release()
{
    ...
    #if defined(CHIP_HAS_CP) && (RAMCP_SIZE > 0)
    cp_pool_init();
    TRACE_IMM(3,"[%s] addblock = %d start %p ", __func__, __cp_mem_pool_end__ - __cp_mem_pool_start__,__cp_mem_pool_start__);
    med_heap_add_block(__cp_mem_pool_start__,__cp_mem_pool_end__ - __cp_mem_pool_start__);
    more_need_buf -= (__cp_mem_pool_end__ - __cp_mem_pool_start__);
    #endif
    TRACE_IMM(2, "more need buf =%d %d",(int)more_need_buf,(int)overlay_text_buf);
    //2048 for med bolck may waste  ram when memlloc
    if(((overlay_text_buf-2048)>=more_need_buf)||(int)more_need_buf<0)
    {
        TRACE_IMM(1,"@%d",__LINE__);
        med_heap_init(&__overlay_text_start__[0], MED_MEM_POOL_SIZE);    
    }else{
        TRACE_IMM(1,"@%d",__LINE__);
...
void bt_IQ_DC_Mismatch_Correction_Release_rx()
{
    ...
    #if defined(CHIP_HAS_CP) && (RAMCP_SIZE > 0)
    cp_pool_init();
    TRACE(3,"[%s] addblock = %d start %p ", __func__, __cp_mem_pool_end__ - __cp_mem_pool_start__,__cp_mem_pool_start__);
    med_heap_add_block(__cp_mem_pool_start__,__cp_mem_pool_end__ - __cp_mem_pool_start__);
    more_need_buf -= (__cp_mem_pool_end__ - __cp_mem_pool_start__);
    #endif
    TRACE_IMM(2, "more need buf =%d %d",(int)more_need_buf,(int)overlay_text_buf);
    ...
    if(((overlay_text_buf-2048)>=more_need_buf)||(int)more_need_buf<0)
    {
        ...
        TRACE_IMM(1, "more need buf =%d %d",more_need_buf,overlay_text_buf);
        more_need_buf =more_need_buf-overlay_text_buf+2048;
        TRACE_IMM(1, "more need buf =%d",more_need_buf);
        med_heap_init(&__overlay_text_start__[0], overlay_text_buf);  
        syspool_get_buff(&g_medMemPool, more_need_buf);
        med_heap_add_block(g_medMemPool,more_need_buf);
    }
...
{
    TRACE(3, "%s Txdc_cal_set: i=%d, q=%d", __func__, g_dc_i_val, g_dc_q_val);
